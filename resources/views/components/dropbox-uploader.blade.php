    ...
    <div class="mb-3">
        ...
        <div class="input-group">
            ...
            <button type="button" 
                    ...
                    id="upload_to_dropbox_btn" 
                    onclick="uploadVideoToDropbox()" 
                    disabled>
        ...
        <a href="{{ url('/app/entertainments/dropbox/connect') }}" style="display:none">Connect Dropbox</a>
        <a href="{{ url('/app/entertainments/dropbox/refresh') }}" style="display:none">Refresh Dropbox Token</a>
    </div>

    <div id="upload_progress" class="progress mb-3" style="display: none;">
        <div class="progress-bar" role="progressbar" style="width: 0%"></div>
    </div>

    <div id="upload_status" class="alert" style="display: none;"></div>
    ...
    <div id="dropbox_file_info" class="card" style="display: none;">
        <div class="card-body">
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <p><strong>{{ __('Dropbox URL') }}:</strong> <span id="dropbox_file_url"></span></p>
...
let dropboxUploadInProgress = false;



function handleVideoFileSelect(input) {
    const file = input.files[0];
    const uploadBtn = document.getElementById('upload_to_dropbox_btn');
    
    if (file) {
        // Validate file type
        if (!file.type.startsWith('video/')) {
            showUploadStatus('Please select a valid video file.', 'danger');
            input.value = '';
        ...
        if (file.size > maxSize) {
            showUploadStatus('File size must be less than 2GB.', 'danger');
            input.value = '';
            uploadBtn.disabled = true;
            return;
        }
...
function uploadVideoToDropbox() {
    if (!selectedVideoFile || dropboxUploadInProgress) {
        return;
    }
    
    dropboxUploadInProgress = true;
    const uploadBtn = document.getElementById('upload_to_dropbox_btn');
    ...
    .then(data => {
        if (data.success) {
            showUploadStatus('Video uploaded to Dropbox successfully!', 'success');
            
            // Update hidden input with Dropbox URL
            // document.getElementById('dropbox_video_url_input').value = data.dropbox_url;
            
            // Show file info
            // document.getElementById('dropbox_file_url').textContent = data.dropbox_url;
            document.getElementById('dropbox_url').value = data.dropbox_url;
            document.getElementById('dropbox_url').textContent = data.dropbox_url;
            ...
            // document.getElementById('video_url_input').value = data.hls_file;
            let drUrl = data.dropbox_url;
            drUrl = drUrl.replace("&dl=0", "&dl=1");
            console.log("hls url :: "+ data.hls_file);
            dropbox_link = drUrl;
    ...
    .catch(error => {
        ...
        showUploadStatus('Upload failed. Please check your connection and try again.', 'danger');
    })
    .finally(() => {
        dropboxUploadInProgress = false;
        uploadBtn.disabled = false;
...
function copyDropboxUrl() {
    const url = document.getElementById('dropbox_file_url').textContent;
...
function testDropboxConnection() {
    ...
    .then(data => {
        if (data.success) {
            showUploadStatus('Dropbox connection successful! Account: ' + data.account.name, 'success');
        } else {
            showUploadStatus('Dropbox connection failed: ' + data.message, 'danger');
        }
    })
    .catch(error => {
        console.error('Connection test error:', error);
        showUploadStatus('Connection test failed. Please check your settings.', 'danger');
    })
    .finally(() => {
...
function checkDropboxConfig() {
    ...
    .then(data => {
        if (data.success) {
            ...
            showUploadStatus(message, config.is_configured && config.token_format_valid ? 'success' : 'warning');
        } else {
            showUploadStatus('Configuration check failed: ' + data.message, 'danger');
        }
    })
    .catch(error => {
        console.error('Configuration check error:', error);
        showUploadStatus('Configuration check failed. Please check your settings.', 'danger');
    })
    .finally(() => {
...
function testFilenameGeneration() {
    ...
    .then(data => {
        if (data.success) {
            ...
            showUploadStatus('Filename test failed: ' + data.message, 'danger');
        }
    })
    .catch(error => {
        console.error('Filename test error:', error);
        showUploadStatus('Filename test failed. Please check your settings.', 'danger');
    })
    .finally(() => {
        testBtn.disabled = false;
        testBtn.innerHTML = originalText;
...
window.initDropboxUploader = function() {
    ...
    document.getElementById('upload_to_dropbox_btn').disabled = true;
    document.getElementById('upload_progress').style.display = 'none';
    document.getElementById('upload_status').style.display = 'none';
